...
import { referenceManifest } from "../src/research/reference-manifest.js";
import { REFERENCE_IDS } from "../src/research/reference-types.js";

const strict = process.argv.includes("--strict");
const sha256Pattern = /^[a-f0-9]{64}$/u;
const ids = referenceManifest.references.map(({ id }) => id);
if (ids.length !== REFERENCE_IDS.length || new Set(ids).size !== REFERENCE_IDS.length || REFERENCE_IDS.some((id) => !ids.includes(id))) {
...
for (const reference of referenceManifest.references) {
  ...
  if (bytes.readUInt32BE(16) !== reference.width || bytes.readUInt32BE(20) !== reference.height) throw new Error(`${reference.id}: dimensions mismatch.`);
}
if (strict && captured !== REFERENCE_IDS.length) throw new Error(`Strict reference check: ${captured}/8 captured; ${8 - captured} pending.`);
console.log(`references: manifest valid; ${captured}/8 captured, ${8 - captured}/8 pending${strict ? " (strict)" : ""}`);
