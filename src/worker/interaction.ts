...
export function hitTest(layout: LayoutSnapshot, view: Vec2,preferredDirection?:"input"|"output"): Hit {
  ...
  for(const control of [...layout.controls.values()].reverse())if(!control.linked){if(control.colorWheelBounds){if(inRect(world,control.colorWheelBounds.plane))return{kind:"color-wheel",id:control.id,region:"plane"};if(inRect(world,control.colorWheelBounds.lightness))return{kind:"color-wheel",id:control.id,region:"lightness"};continue;}const ramp=hitRamp(control,world,tolerance);if(ramp)return ramp;for(const field of control.numericFields){if(inRect(world,field.decrement))return{kind:"control-step",id:control.id,component:field.component,direction:-1};if(inRect(world,field.increment))return{kind:"control-step",id:control.id,component:field.component,direction:1};}if(inRect(world,control.bounds)){const component=control.subfields.find(field=>inRect(world,field.bounds))?.index??0;return{kind:"control",id:control.id,component};}}
