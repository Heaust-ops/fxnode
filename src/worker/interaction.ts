...
export function hitTest(layout: LayoutSnapshot, view: Vec2,preferredDirection?:"input"|"output"): Hit {
  ...
  const topNode=layout.drawOrder.slice().reverse().map(id=>layout.nodes.get(id)).find(node=>node!==undefined&&node.kind!=="frame"&&inRect(world,node.bounds));
  for(const control of [...layout.controls.values()].reverse())if(!control.linked&&(!topNode||control.nodeId===topNode.id)){if(control.colorWheelBounds){if(inRect(world,control.colorWheelBounds.plane))return{kind:"color-wheel",id:control.id,region:"plane"};if(inRect(world,control.colorWheelBounds.lightness))return{kind:"color-wheel",id:control.id,region:"lightness"};continue;}const ramp=hitRamp(control,world,tolerance);if(ramp)return ramp;for(const field of control.numericFields){if(inRect(world,field.decrement))return{kind:"control-step",id:control.id,component:field.component,direction:-1};if(inRect(world,field.increment))return{kind:"control-step",id:control.id,component:field.component,direction:1};}if(inRect(world,control.bounds)){const component=control.subfields.find(field=>inRect(world,field.bounds))?.index??0;return{kind:"control",id:control.id,component};}}
  for(const id of layout.drawOrder.slice().reverse()){const node=layout.nodes.get(id);if(node?.kind!=="reroute")continue;const center={x:node.bounds.x+node.bounds.width/2,y:node.bounds.y-node.bounds.height/2},distance=Math.hypot(world.x-center.x,world.y-center.y),core=Math.max(G.reroute,7/layout.transform.zoom),halo=core+8/layout.transform.zoom;if(distance>core&&distance<=halo)return{kind:"node",id};}
...
export function hitNode(layout: LayoutSnapshot, view: Vec2): NodeId | undefined {
  const hit=hitTest(layout,view);return hit.kind==="control"||hit.kind==="control-step"||hit.kind==="ramp"||hit.kind==="color-wheel"?layout.controls.get(hit.id)?.nodeId:hit.kind==="socket"?layout.sockets.get(hit.id)?.nodeId:hit.kind==="node"||hit.kind==="collapse"||hit.kind==="resize"||hit.kind==="frame-header"||hit.kind==="frame-body"?hit.id:undefined;
}
...
export const clampResize=(layout:LayoutSnapshot,id:NodeId,world:Vec2):Vec2|undefined=>{const n=layout.nodes.get(id);if(!n||n.kind!=="node")return;return{x:Math.min(G.maxWidth,Math.max(n.minimumSize.x,world.x-n.bounds.x)),y:Math.max(n.minimumSize.y,n.bounds.y-world.y)};};
