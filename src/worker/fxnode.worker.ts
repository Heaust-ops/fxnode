...
function cancelGesture(): boolean { const active=!!session.drag||!!session.scrub||!!session.rampDrag||!!session.modalMove||!!session.box||!!session.linkDrag||!!session.resize||!!session.pan||!!session.controlEdit||!!gestureLayout||session.previewValues.size>0||session.previewPositions.size>0||session.previewSizes.size>0;if(session.box)session.selectedNodes=new Set(session.box.checkpoint);delete session.drag;delete session.scrub;delete session.rampDrag;delete session.modalMove;delete session.box;delete session.linkDrag;delete session.resize;delete session.parentHighlight;delete session.pan;delete session.controlEdit;session.previewValues.clear();gestureLayout=undefined;session.previewPositions.clear();session.previewSizes.clear();return active; }
