...
function beginColorPicker(id:string,anchor:{x:number;y:number;width:number;height:number},rgba:Rgba,target:{kind:"control"}|{kind:"ramp-stop";stopId:string;original:ColorRamp}):void{const model=oklabToOklch(srgbToOklab([rgba[0],rgba[1],rgba[2]]));session.colorPicker={layout:layoutColorPicker(anchor,{x:viewport.width,y:viewport.height}),controlId:id,target,model,rgba};scheduler.request();}
