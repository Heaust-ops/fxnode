...
const scheduler = new RenderScheduler((frameId,renderId) => {
  ...
  if(session.colorPicker)paintColorPicker(context,session.colorPicker.layout,session.colorPicker.model,session.colorPicker.rgba,viewport.dpr);
...
function openRampPicker(id:string,ramp:ColorRamp):void{const control=currentLayout?.controls.get(id),bounds=control?.rampBounds;if(!control||!bounds)return;const stop=activeStop(id,ramp),topLeft=worldToView({x:bounds.color.x,y:bounds.color.y},currentLayout!.transform),anchor={x:topLeft.x,y:topLeft.y,width:bounds.color.width*.2*currentLayout!.transform.zoom,height:bounds.color.height*currentLayout!.transform.zoom},rgba=stop.color as Rgba,model=oklabToOklch(srgbToOklab([rgba[0],rgba[1],rgba[2]]));session.colorPicker={layout:layoutColorPicker(anchor,{x:viewport.width,y:viewport.height}),controlId:id,stopId:stop.id,original:ramp,model,rgba};scheduler.request();}
function updatePicker(position:{x:number;y:number}):void{const picker=session.colorPicker;if(!picker?.drag)return;const rect=picker.layout[picker.drag.region];if(picker.drag.region==="plane"){const radius=rect.width/2,dx=(position.x-(rect.x+radius))/radius,dy=((rect.y+radius)-position.y)/radius,length=Math.min(1,Math.hypot(dx,dy)),h=Math.atan2(dy,dx),c=length*maxSrgbChroma(picker.model.l,h);picker.model={...picker.model,c,h};}else if(picker.drag.region==="lightness"){const l=1-Math.max(0,Math.min(1,(position.y-rect.y)/rect.height));picker.model={...picker.model,l,c:Math.min(picker.model.c,maxSrgbChroma(l,picker.model.h))};}else picker.rgba=[picker.rgba[0],picker.rgba[1],picker.rgba[2],1-Math.max(0,Math.min(1,(position.y-rect.y)/rect.height))];const rgb=mapOklchToSrgb(picker.model);picker.rgba=[rgb[0],rgb[1],rgb[2],picker.rgba[3]];const ramp=setRampColor(picker.original,picker.stopId,picker.rgba);session.previewValues.set(picker.controlId,{kind:"json",value:ramp as unknown as import("../core/types.js").JsonValue});scheduler.request();}
function finishPicker(commitValue:boolean):void{const picker=session.colorPicker;if(!picker)return;const preview=session.previewValues.get(picker.controlId) as {kind?:unknown;value?:unknown}|undefined;session.previewValues.delete(picker.controlId);delete session.colorPicker;if(commitValue&&preview?.kind==="json"&&isColorRamp(preview.value))commitRamp(picker.controlId,preview.value,picker.stopId);else scheduler.request();}
...
function input(event: Extract<import("../browser/protocol.js").WorkerRequest,{type:"input"}>["event"],nodeMenuRequestId?:string): void {
  ...
  if (event.kind === "wheel") { const next=zoomAt(currentTransform(),event.position,event.delta.y);session.zoom=next.zoom;session.cameraCenter=next.center;refreshLayout();scheduler.request();return; }
  ...
  if(session.colorPicker){
    ...
    if(picker.drag?.pointerId===event.pointerId){if(event.phase==="move")updatePicker(event.position);else finishPicker(event.phase==="up");return;}
    if(event.phase==="down"&&event.button===0){const region=(["plane","lightness","alpha"] as const).find(name=>containsView(event.position,picker.layout[name]));if(region){picker.drag={pointerId:event.pointerId,region};updatePicker(event.position);}else finishPicker(false);return;}
    return;
  }
