...
export function paintColorPicker(context:OffscreenCanvasRenderingContext2D,layout:ColorPickerLayout,model:Oklch,rgba:Rgba,hsv:readonly number[],edit?:{field:string;index:number;buffer:string;invalid:boolean},dpr=1):void{
  ...
  const y=layout.alpha.y+layout.alpha.height*(1-rgba[3]);context.strokeStyle="#fff";context.strokeRect(layout.alpha.x-2,y-2,layout.alpha.width+4,4);context.font="12px sans-serif";context.textBaseline="middle";context.textAlign="center";context.fillStyle="#eee";context.fillText("✓",layout.confirm.x+12,layout.confirm.y+12);const hex="#"+rgba.map(v=>Math.round(Math.max(0,Math.min(1,v))*255).toString(16).padStart(2,"0")).join("").toUpperCase();for(const[rects,values,name,labels]of [[layout.rgba,rgba.map(v=>v.toFixed(3)),"rgba","RGBA"],[layout.hsv,[hsv[0]!.toFixed(1),hsv[1]!.toFixed(3),hsv[2]!.toFixed(3)],"hsv","HSV"]] as const)rects.forEach((r,i)=>{context.fillStyle="#292c33";context.fillRect(r.x,r.y,r.width,r.height);context.strokeStyle=edit?.field===name&&edit.index===i?(edit.invalid?"#e55":"#f5a623"):"#555";context.strokeRect(r.x+.5,r.y+.5,r.width-1,r.height-1);context.fillStyle="#eee";context.fillText(edit?.field===name&&edit.index===i?edit.buffer:`${labels[i]} ${values[i]}`,r.x+r.width/2,r.y+r.height/2);});context.fillStyle="#292c33";context.fillRect(layout.hex.x,layout.hex.y,layout.hex.width,layout.hex.height);context.strokeStyle=edit?.field==="hex"?(edit.invalid?"#e55":"#f5a623"):"#555";context.strokeRect(layout.hex.x+.5,layout.hex.y+.5,layout.hex.width-1,layout.hex.height-1);context.fillStyle="#eee";context.fillText(edit?.field==="hex"?edit.buffer:`HEX ${hex}`,layout.hex.x+layout.hex.width/2,layout.hex.y+layout.hex.height/2);context.restore();
}
