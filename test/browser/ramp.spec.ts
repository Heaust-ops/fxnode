...
type RampWindow = Window &
  typeof globalThis & { rampTest: { api: FxNode | null; ready: Promise<void> }; rampEvents: Events };
...
async function open(page: Page) {
  ...
  await page.evaluate(() => {
    ...
    w.rampTest.api!.onMutations((e) => w.rampEvents.mutations.push(e.version));
    w.rampTest.api!.onSnapshots((e) => w.rampEvents.snapshots.push(e.version));
  });
...
const state = (page: Page): Promise<State> =>
  page.evaluate(async () => {
    const w = window as unknown as RampWindow;
    return {
      snapshot: await w.rampTest.api!.getState(),
      layout: await w.rampTest.api!.save(),
...
test("handle drag previews pixels, commits once, and Escape/RMB cancel", async ({ page }) => {
  ...
  await page.evaluate(() => (window as unknown as RampWindow).rampTest.api!.whenRendered());
...
test("mode, interpolation, and hue controls cycle legal values and survive save/load", async ({ page }) => {
  ...
  await page.evaluate(
    (layout: unknown) => (window as unknown as RampWindow).rampTest.api!.load(layout),
    saved as unknown,
  );
