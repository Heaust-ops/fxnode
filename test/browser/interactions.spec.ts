...
test("worker gestures stay transient and commit exactly one paired event",async({page})=>{
  ...
  await canvas.click({position:{x:320,y:160}});expect((await snapshot()).version).toBe(original.version);expect(await page.evaluate(()=>(window as typeof window & {gestureEvents:{m:number;s:number}}).gestureEvents)).toEqual({m:0,s:0});
  const bounds=await canvas.boundingBox();if(!bounds)throw new Error("canvas bounds missing");await canvas.hover({position:{x:320,y:160}});await page.mouse.down();await page.mouse.move(bounds.x+350,bounds.y+180,{steps:4});expect((await snapshot()).nodes.find(n=>n.id==="math")?.position).toEqual({x:-300,y:170});await page.mouse.up();
  ...
  await canvas.hover({position:{x:350,y:180}});await page.mouse.down();await page.mouse.move(bounds.x+390,bounds.y+220);await canvas.press("Escape");await page.mouse.up();await canvas.click({position:{x:30,y:30},button:"right"});expect((await snapshot()).version).toBe(1);
...
test("non-isolated hosts retain the ordered pointer message fallback",async({page})=>{
  ...
  const canvas=page.locator("#graph"),bounds=await canvas.boundingBox();if(!bounds)throw new Error("canvas bounds missing");await canvas.hover({position:{x:320,y:160}});await page.mouse.down();await page.mouse.move(bounds.x+350,bounds.y+180,{steps:4});await page.mouse.up();
