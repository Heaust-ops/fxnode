...
test("Image Texture conditional blend and persisted texture modes",async({page})=>{
  const canvas=await open(page),a=await saved(page),flatPixels=await pixels(page,110,410,300,300);
  ...
  await canvas.click({position:P.image.projection});const box=await saved(page);paired(a,box);expect(parameter(box,"image-texture","projection")).toEqual({kind:"string",value:"Box"});expect(await pixels(page,110,410,300,300)).not.toBe(flatPixels);
...
test("Compositor Image movie rows, resource editing and output contract",async({page})=>{
  ...
  await canvas.click({position:P.compositor.resource});await page.keyboard.type("clip.exr");await page.keyboard.press("Enter");let b=await saved(page);paired(a,b);expect(parameter(b,"compositor-image","image")).toEqual({kind:"string",value:"clip.exr"});
  await canvas.click({position:P.compositor.resource});await page.keyboard.type(".bad");await page.keyboard.press("Escape");paired(b,await saved(page),0);
