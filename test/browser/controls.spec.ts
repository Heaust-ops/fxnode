...
const state = (page: ControlPage) =>
  page.evaluate(async () => ({
    snapshot: await window.controlTest.api!.getState(),
    layout: await window.controlTest.api!.save(),
...
test("Backspace resets defaults, undo restores, and linked socket is inert", async ({ page }) => {
  ...
  await p.evaluate(() => window.controlTest.api!.undo());
...
test("muting a link reveals its default control and unmuting hides it", async ({ page }) => {
  const p = await open(page),
    ...
    a = await state(p);
  await p.evaluate(() => {
    const id = window.controlTest.api!.save().then((layout) => layout.links[0]!.id);
    return id.then((link) => window.controlTest.api!.dispatch({ type: "link.mute", id: link, value: true }));
  });
  await p.evaluate(() => window.controlTest.api!.whenRendered());
  ...
  await p.evaluate(() => {
    const id = window.controlTest.api!.save().then((layout) => layout.links[0]!.id);
    return id.then((link) => window.controlTest.api!.dispatch({ type: "link.mute", id: link, value: false }));
  });
  await p.evaluate(() => window.controlTest.api!.whenRendered());
